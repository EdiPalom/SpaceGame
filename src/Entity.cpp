...
Entity::Entity(std::string name):visible(false),b_destroy(false),type(NONE),collision(false)
{
    this->id = p_layer;
    this->name = name;
}

Entity::Entity(SDL_Texture *texture, sf::Vector2i size, sf::Vector2f position, sf::Vector2f direction):visible(true),b_destroy(false),type(NONE),collision(false)
{
    this->sprite.x = 0;
    this->sprite.y = 0;
